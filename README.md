...
本地历史按流逐行解析并即时聚合，不会把完整 JSONL 和多份全量事件数组同时加载到内存。异常单行默认超过 64 MiB 时会整行跳过并继续扫描；可用 `TOKENRANK_MAX_JSONL_LINE_BYTES` 调整上限。普通 JSON 文档默认限制为 64 MiB，可用 `TOKENRANK_MAX_JSON_DOCUMENT_BYTES` 调整。
