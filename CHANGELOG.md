...
- `preview` 完整扫描成功后会在本机短暂缓存仅含日期、工具、模型和 Token 聚合的结果；30 分钟内接续的完整 `upload` 会复用该快照并按原有上传日期窗口筛选，跳过重复读取本地历史。缓存失效、筛选参数不同或扫描不完整时自动安全重扫。

## 0.2.1 - 2026-07-21

- 将 `preview` 重做为登录前的首次体验：扫描前展示 TokenRank 大字标，扫描后优先呈现 Token 总量、活跃天数、工具/模型数、UTC 日期范围与峰值日期。
- Logo 与命令之间新增留白，并在交互式终端加入约 0.5 秒的逐行点亮动画；可继续通过 `TOKENRANK_NO_ANIMATION=1` 关闭，`--json` 与非 TTY 输出契约保持不变。
- 将容易误读的跨日期压缩曲线改为最近活跃日横条和明确数值；AI 工具与模型占比移除无含义的暗色底轨，小于 1% 的占比不再显示成 0%。
- 将结尾操作改为分步首次引导：未连接时明确说明使用 X 登录、复制私人连接命令及上传每日聚合，已连接时展示上传和查看排名入口，并始终强调本次预览没有上传数据。
